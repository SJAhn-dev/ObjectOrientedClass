...
Number *Complex::add(Number * num) {
	...
	if(num->types() == INTEGER){
		Integer *tg = dynamic_cast<Integer*>(num);
		double _val = tg->val();
		delete(tg);
		Complex *cpx = new Complex(_val + this->value.first , this->value.second);
		delete(this);
		return cpx;
	} else if(num->types() == FLOAT){
		Float *tg = dynamic_cast<Float*>(num);
		double _val = tg->val();
		delete(tg);
		Complex *cpx = new Complex(_val + this->value.first , this->value.second);
		delete(this);
		...
		Complex *tg = dynamic_cast<Complex*>(num);
		std::pair<double,double> _val = tg->val();
		Complex *cpx = new Complex(this->value.first + _val.first , this->value.second + _val.second);
		delete(this);
		delete(tg);
		return cpx;
	}
...
Number *Complex::sub(Number * num) {
	...
	if(num->types() == INTEGER){
         ...
         double _val = tg->val();
         delete(tg);
         Complex *cpx = new Complex(this->value.first - _val, this->value.second);
         delete(this);
         return cpx;
    } else if(num->types() == FLOAT){
         Float *tg = dynamic_cast<Float*>(num);
...
Number *Complex::mul(Number * num) {
	...
	if(num->types() == INTEGER){
         ...
         double _val = tg->val();
         delete(tg);
         Complex *cpx = new Complex(_val * this->value.first , _val * this->value.second);
         delete(this);
         return cpx;
     } else if(num->types() == FLOAT){
         Float *tg = dynamic_cast<Float*>(num);
         ...
         Complex *tg = dynamic_cast<Complex*>(num);
         std::pair<double,double> _val = tg->val();
		 double real = this->value.first * _val.first;
		 double fake = (this->value.first * _val.second) + (this->value.second * _val.first) + (this->value.second * _val.second);
         Complex *cpx = new Complex(real,fake);
         delete(this);
         delete(tg);
		 return cpx;
     }
...
Number *Complex::div(Number * num) {
	...
	if(num->types() == INTEGER){
		...
		double _val = tg->val();
        delete(tg);
        Complex *cpx = new Complex(this->value.first / _val , _val * this->value.second);
        delete(this);
        return cpx;
    } else if(num->types() == FLOAT){
        Float *tg = dynamic_cast<Float*>(num);
