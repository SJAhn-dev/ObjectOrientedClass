...
Number *Integer::add(Number * num) {
    ...
    if (num->types() == INTEGER) {
		...
		int _val = tg->val();
		this->value = _val + this->value;
		delete(tg);
		return this;
    } else if (num->types() == FLOAT) {
		Float *tg = dynamic_cast<Float*>(num);
		double _val = tg->val();
		double _thisval = this->value;
		tg->set_val(_val+_thisval);
		delete(this);
		return tg;
		...
		double _thisval = this->value;
		_thisval = _thisval + _val.first;
		Complex *newCom = new Complex(_thisval , _val.second);
		delete(this);
		delete(tg);
		return newCom;
    } else {
...
Number *Integer::mul(Number * num) {
    ...
    if (num->types() == INTEGER) {
        ...
        double _thisval = this->value;
        _thisval = _thisval * _val.first;
        Complex *newCom = new Complex(_thisval , _val.second * _thisval );
        delete(this);
        delete(tg);
        return newCom;
