...
shared_ref::shared_ref(student* student) {
	...
	if(ptr_map.find(student) != ptr_map.end()){
		if(ptr_map.find(student)->second==nullptr){
			this->_mgr = new mgr(student);
			ptr_map.find(student)->second = this->_mgr;
		} else{
			this->_mgr = ptr_map.find(student)->second;
			this->increase();
		}
...
void shared_ref::release() {
	...
	if(this->_mgr != nullptr){
		...
		if(this->_mgr->count==0){
			ptr_map.find(this->_mgr->ptr)->second = nullptr;
			delete(this->_mgr);
		}
		this->_mgr = empty_mgr;
	}
